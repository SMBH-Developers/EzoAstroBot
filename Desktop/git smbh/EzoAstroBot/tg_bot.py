...
astro_advice_photo = "AgACAgIAAxkBAAE3CsNkRsc_16nRiirCDgL2Yzb2xR58mAAC8ssxG9D-MEry1dSDxjmqtwEAAwIAA3kAAy8E"
astrology_is_photo = "AgACAgIAAxkBAAE3CtFkRsdVCpuqUXb2m_7NUuhSs2qxmwAC9MsxG9D-MErfwLuLkCXPTwEAAwIAA3kAAy8E"
lune_horoscope_photo = "AgACAgIAAxkBAAE3CyFkRsfIuFoM0u2Az5jFvJWI2ES75wAC-csxG9D-MEr1Ot3X3Y1BgwEAAwIAA3kAAy8E"
year_horoscope_photo = "AgACAgIAAxkBAAE3CydkRsfUszAkfGcfwtqkM-SVIz__TQAC-ssxG9D-MErRcZGHrCAgMgEAAwIAA3kAAy8E"
...
@dp.message_handler(lambda message: message.from_user.id == 1188441997, content_types=['video'], state='*')
async def get_photo_from_me(message: types.Message, state: FSMContext):
...
async def which_horoscope(message: types.Message, state) -> None:
    if message.text == '✨Получить бесплатный гороскоп':
        ...
        if not bool(user_date):
            ...
            zodiac = get_zodiac_sign(day, month, language='ru_RU')
            user_choose = await db.get_horoscope_text_index(message.from_user.id)
            await message.answer_photo(year_horoscope_photo,
...
async def study_menu_dispatcher(message: types.Message, state: FSMContext):
    if message.text in ('🙏Получить бесплатный гороскоп', '✨Получить бесплатный гороскоп на год'):
        ...
        if not bool(user_date):
            ...
            zodiac = get_zodiac_sign(day, month, language='ru_RU')
            user_choose = await db.get_horoscope_text_index(message.from_user.id)
            await message.answer_photo(photo=year_horoscope_photo,
                                       caption=generate_beautiful_text('year', zodiac, user_date, user_choose),
                                       reply_markup=markups.to_menu_mrkup,
                                       parse_mode='html')
            await state.set_state(States.back_state.state)
        ...
        await message.answer_photo(astro_advice_photo, text, reply_markup=markups.mrkup_for_every_study_btn)
    elif message.text == '✨Что такое астрология?':
        await message.answer_photo(astrology_is_photo, study_menu_texts[message.text],
...
async def send_analyze_of_answers(chat_id, text_to_send):
    ...
    await bot.send_photo(chat_id, photo=year_horoscope_photo, caption=text_to_send, parse_mode='html')
...
async def sending_message_24_h():
    while True:
        ...
        mrkup = types.InlineKeyboardMarkup()
        mrkup.add(types.InlineKeyboardButton("🔆Бесплатный астрологический разбор", url="https://t.me/your_mentoor"))
...
async def sending_message_48_h():
    while True:
        ...
        mrkup = types.InlineKeyboardMarkup()
        mrkup.add(types.InlineKeyboardButton("Забрать подарок🎁", url="https://t.me/your_mentoor"))
...
async def sending_message_72h():
    while True:
        ...
        kb.add(types.InlineKeyboardButton("Получить разбор🔱", url="https://t.me/your_mentoor"))
...
async def on_startup(_):
    # asyncio.create_task(nov25_marathon())
