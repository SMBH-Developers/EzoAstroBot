...
async def get_users_2h_autosending():
    async with async_session() as session:
        users = (await session.execute(select(User.id).where(
            (func.now() - User.registration_date).between(timedelta(minutes=127), timedelta(minutes=1000)),
...
#
#
# async def get_users_black_friday_sending() -> list[int]:
#     query = select(User.id).\
#         where(User.black_friday_sending.is_(None), (User.registration_date + timedelta(days=130)) < datetime.now()).\
...
#
#
# async def set_black_friday(id_: int, bf_state: str | None):
#     query = update(User).where(User.id == id_).values(black_friday_sending=func.now(), bf_state=bf_state)
#     async with async_session() as session:
#         await session.execute(query)
#         await session.commit()
#
#
# async def get_users_black_friday_sending_2() -> list[int]:
#     query = select(User.id).\
#         where(User.black_friday_sending_2.is_(None),
#               (func.now() - User.black_friday_sending) > timedelta(hours=7)).\
#         order_by(User.registration_date.desc())
#     async with async_session() as session:
#         users = (await session.execute(query)).scalars().all()
#     return users
